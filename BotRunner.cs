    ...
    public static class BotRunner {
        public static async Task RunFrom(CrossBotConfig config, CancellationToken cancel) {
            ...
            if (config.TWITCH_ENABLED) {
                var twitch = new TwitchBot(config);
            } 
            Globals.Self = sys;
            Globals.Bot = bot;
            LogUtil.LogInfo("Starting Discord.", bot.Connection.IP);
#pragma warning disable 4014
            Task.Run(() => sys.MainAsync(config.DISCORD_TOKEN, cancel), cancel);
#pragma warning restore 4014

            if (config.SKIP_CONSOLE_BOT_CREATION) {
            ...
            if (task.IsFaulted) {
                if (task.Exception == null) {
                    ...
                    foreach (var ex in task.Exception.InnerExceptions) {
                        ...
                        if (st != null) LogUtil.LogError(st, bot.Connection.IP);
                    }
                }
            } else {
