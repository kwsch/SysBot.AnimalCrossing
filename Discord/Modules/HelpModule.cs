        ...
        public async Task HelpAsync() {
            ...
            foreach (var module in _service.Modules) {
                ...
                foreach (var cmd in module.Commands) {
                    ...
                    if (cmd.Attributes.Any(z => z is RequireOwnerAttribute) && owner != uid) continue;
                    if (cmd.Attributes.Any(z => z is RequireSudoAttribute) && !Globals.Bot.Config.CanUseSudo(uid)) continue;

                    mentioned.Add(name);
                    var result = await cmd.CheckPreconditionsAsync(Context).ConfigureAwait(false);
                    if (result.IsSuccess) description += $"{cmd.Aliases[0]}\n";
                }
