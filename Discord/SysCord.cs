...
namespace SysBot.AnimalCrossing {
    public sealed class SysCord {
        ...
        public async Task MainAsync(string apiToken, CancellationToken token) {
            ...
            var game = Bot.Config.DISCORD_NAME;
            if (!string.IsNullOrWhiteSpace(game)) await _client.SetGameAsync(game).ConfigureAwait(false);
        ...
        private async Task Ready() {
            try { await Bot.ChannelReady(_client).ConfigureAwait(false); } catch { Console.WriteLine("Queue channel required permissions you did not have!"); }
        }
        private async Task HandleMessageAsync(SocketMessage arg) {
            ...
            int pos = 0;
            if (msg.HasStringPrefix(Bot.Config.DISCORD_PREFIX, ref pos)) {
        ...
        private async Task<bool> TryHandleCommandAsync(SocketUserMessage msg, int pos) {
            ...
            if (msg.Content == $"{Globals.Bot.Config.DISCORD_PREFIX}dodo" || msg.Content == $"{Globals.Bot.Config.DISCORD_PREFIX}code") {
                if (Globals.Bot.IsQueueFull()) await msg.Channel.SendMessageAsync($"Queue is currently full. Please wait!~").ConfigureAwait(false);
                Globals.Bot.ShowQueue ^= true;
                await msg.Channel.SendMessageAsync($"Dodo Code: {Globals.Bot.DodoCode}.\n(Current Drop-Limit: {Globals.Bot.MaxRequestCount})").ConfigureAwait(false);
                return true;
            }
            var mgr = Bot.Config;
        ...
        private async Task MonitorStatusAsync(CancellationToken token) {
            ...
            while (!token.IsCancellationRequested) {
                ...
                if (gap <= TimeSpan.Zero) {
                    var idle = !Bot.Config.DISCORD_ACCEPTINGDROPS ? UserStatus.DoNotDisturb : UserStatus.Idle;
                    if (idle != state) {
                        state = idle;
                ...
                var active = !Bot.Config.DISCORD_ACCEPTINGDROPS ? UserStatus.DoNotDisturb : UserStatus.Online;
                if (active != state) {
                    state = active;
                    await _client.SetStatusAsync(state).ConfigureAwait(false);
                }
